 ...
 *
 */
@Service("textEventTargetAdapter")
public class TextEventTargetAdapter implements MessageRender {
	...
	public AbstractWeixinMessage render(AbstractWeixinMessage message) {
		logger.debug("process text event target...");
		if( message instanceof FromWeixinTextMessage){
			FromWeixinTextMessage from = (FromWeixinTextMessage)message;
			String word = from.getContent();
			
			//context 用于获取当前用户的id  wujiajun
			...
			List<WeixinKeyword> keywords = weixinKeywordDao.findByUidAndWord(new Long(context.getUid()), word);
			WeixinKeyword keyword = null;
			if(keywords.size() > 0 ){
				keyword = keywords.get(0);
			}
			
			if( null == keyword) { 	System.out.println("keyword查询没有东西"); return null;}
