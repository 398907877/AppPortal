...
<form:form id="form" method="post" modelAttribute="supplyDemand" cssClass="form-horizontal" action="${ctx}/mgr/supply/update"  enctype="multipart/form-data">
	<div class="form-group">
		<label class="col-sm-2 control-label" for="categoryId">供求类型:</label>
		...
		<select id="selectType" name="type" style="min-width:240px;" class="populate placeholder select2-offscreen se2" tabindex="-1" title="">
				...
				<option value="0" selected="selected">供（提供给他人信息）</option>
				<option value="1">求（想要获得的信息）</option>
				</c:if>
				<c:if test="${supplyDemand.type eq 1}">
				<option value="0">供（提供给他人信息）</option>
				<option value="1" selected="selected">求（想要获得的信息）</option>
				</c:if>
		 </select>
	    </div>
	</div>
	...
	<div class="form-group">
	<label class="col-sm-2 control-label" for="categoryId">所属行业:</label>
     <div class="col-sm-10 controls">
		...
		<div class="form-group">
			...
			<a class="btn btn-primary" href="<%=request.getContextPath()%>/mgr/supply" class="btn">返回产品主页</a>
			</div>
		</div>
</form:form>
</div>
	...
	$(function(){
		$("#typeId").select2("val","${supplyDemand.typeId}")
		$(".display").hide();
