...
<body>
   <div class="row">
	...
	<div >
		...
		<form:form id="eventForm" method="post" modelAttribute="event" cssClass="form-horizontal" role="form" action="${ctx}/mgr/events/update"  enctype="multipart/form-data">
		    <form:hidden path="id" />
...
<div class="form-group" id="picFile">
     <label class="col-sm-2 control-label" for="file-input">海报:</label>
<div class="col-sm-10 controls" id="uploadpictureid">
	<div data-provides="fileinput" class="fileinput <c:if test='${event.poster == null && event.poster==""}'>fileinput-new</c:if> <c:if test='${event.poster != null && event.poster!= ""}'>fileinput-exists</c:if>">
				...
				<div  class="fileinput-preview fileinput-exists thumbnail" style="max-width: 300px; max-height: 150px;">
					<c:if test='${event.poster != null && event.poster != ""}'><img src="${event.poster}" style="max-width: 300px;max-height: 150px;"/></c:if>
				</div>
				<div>
					<span class="btn btn-default btn-file">
					...
					<input type="hidden" value="${event.poster}" id="poster" name="poster"/>
				</div>
				</div>
	</div>
</div>
